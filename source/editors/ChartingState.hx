...
class ChartingState extends MusicBeatState
{
	...
	//here ya go giorno - Cyachao
	var fakeBf:Boyfriend;
	...
	**/
	...
	override function create()
	{
		...
		fakeBf = new Boyfriend(900, 300, 'bf');
		fakeBf.setGraphicSize(Std.int(fakeBf.width * 0.7), Std.int(fakeBf.height * 0.7));	
		fakeBf.scrollFactor.set();
		add(fakeBf);
		fakeBf.dance();

		fakeDad = new Character(50, 300, 'dad');
		fakeDad.setGraphicSize(Std.int(fakeDad.width * 0.7), Std.int(fakeDad.height * 0.7));
		fakeDad.scrollFactor.set();
		add(fakeDad);
		fakeDad.dance();
		fakeBf.debugBf = true;
		fakeDad.debugBf = true;
		if (FlxG.save.data.chart_hideGoobers) {
	...
	var hideGoobers:FlxUICheckBox = null;
	...
	function addSongUI():Void
	{
		...
		UI_songDiff = new FlxUIInputText(80, 105, 70, CoolUtil.difficultyString().toLowerCase(), 8);
		...
		{
			openSubState(new Prompt('This action will clear current progress.\n\nProceed?', 0, function(){loadJson(UI_songTitle.text.toLowerCase()); }, null,ignoreWarnings));
		});
	...
	function addChartingUI() {
		...
		hideGoobers = new FlxUICheckBox(100, 150, null, null, 'Hide Singers', 100,
			function() {
				FlxG.save.data.chart_hideGoobers = hideGoobers.checked;
			}
		);
		if (FlxG.save.data.chart_hideGoobers == null) FlxG.save.data.chart_hideGoobers = false;
		hideGoobers.checked = FlxG.save.data.chart_hideGoobers;
		...
		//tab_group_chart.add(new FlxText(hideGoobers.x, hideGoobers.y - 15, 0, 'Hide Chars'));
		...
		tab_group_chart.add(hideGoobers);
				...
				if(note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1) {
					...
					if(note.noteData > -1) {
						if (note.mustPress) {
							...
							fakeBf.playAnim(animToPlay, true);
							fakeBf.holdTimer = 0;
						} else{
							var animToPlay:String = singAnimations[Std.int(Math.abs(note.noteData))] + note.animSuffix;
							fakeDad.playAnim(animToPlay, true);
							fakeDad.holdTimer = 0;
						}
					}
	...
	*/
	...
	function updateHeads():Void
	{
		...
		fakeBf = new Boyfriend(900, 300, _song.player1);
		fakeBf.setGraphicSize(Std.int(fakeBf.width * 0.7), Std.int(fakeBf.height * 0.7));
		fakeBf.scrollFactor.set();
		add(fakeBf);
		fakeBf.dance();

		fakeDad = new Character(50, 300, _song.player2);
		fakeDad.setGraphicSize(Std.int(fakeDad.width * 0.7), Std.int(fakeDad.height * 0.7));
		fakeDad.scrollFactor.set();
		add(fakeDad);
		fakeDad.dance();
	...
	private function addNote(strum:Null<Float> = null, data:Null<Int> = null, type:Null<Int> = null):Void
	{
		...
		if (daType == 7) {
			FlxG.sound.play(Paths.sound('honk'), 0.2, false);
		}
	...
	function loadJson(song:String):Void
	{
		...
		//yes - Cyachao
		var thisThing:String = UI_songDiff.text.toLowerCase();
		var lcSong:String = song.toLowerCase();
		var formattedSong:String = Paths.formatToSongPath(lcSong);
		var realThing:String = 'test';
		//BACKTRACKING BULLCRAP FROM SONG.HX TO CONFIRM

		if (thisThing == '' || thisThing == 'normal') {
			realThing = Paths.json(formattedSong + '/' + formattedSong);
			trace("Looking for: " + realThing);
			if (FileSystem.exists(realThing)) {
				PlayState.SONG = Song.loadFromJson(song.toLowerCase(), song.toLowerCase());
				MusicBeatState.resetState();
			} else {
				//MusicBeatState.resetState();
				UI_songTitle.text = "Error!";
				//Main.showErrorAnalysis(realThing);
			}
		} else {
			realThing = Paths.json(formattedSong + '/' + formattedSong + "-" + thisThing);
			trace("Looking for (abnormal difficulty): " + realThing);
			if (FileSystem.exists(realThing)) {
				PlayState.SONG = Song.loadFromJson(song.toLowerCase() + "-" + thisThing, song.toLowerCase());
				MusicBeatState.resetState();
			} else {
				UI_songTitle.text = 'Error!';
				//Main.showErrorAnalysis(realThing);
				//  Main.openFolder(Paths.json(formattedSong + '/' + formattedSong));
			}
		}
	}
